...
onMounted(async () => {
  ...
  const [featuredQuestions, latestQuestions, homeStats] = await Promise.all([
    questionApi.list({ featured: 1, page_size: 3 }),
    questionApi.list({ page_size: 3 }),
    homeApi.stats(),
  ])
  featured.value = [...featuredQuestions.items, ...latestQuestions.items]
